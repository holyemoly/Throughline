      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        {loading ? (
          ...
          <>
            ...
            {/* API Usage */}
            <div>
              <div style={{ fontSize: '10px', color: 'var(--text-dim)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '10px' }}>api usage (7 days)</div>
              {usage?.error || !usage?.data ? (
                <p style={{ color: 'var(--text-dim)', fontSize: '12px', fontStyle: 'italic' }}>usage data unavailable</p>
              ) : (
                <>
                  {totalCost !== null && (
                    <div style={{ padding: '12px 14px', borderRadius: '12px', background: 'var(--bg-3)', border: '1px solid var(--border)', marginBottom: '12px' }}>
                      <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '4px' }}>estimated spend</div>
                      <div style={{ fontSize: '22px', fontFamily: 'Cormorant Garamond, serif', fontWeight: 300, color: 'var(--purple)' }}>${totalCost.toFixed(4)}</div>
                    </div>
                  )}
                  {usage.data.slice(0, 7).map((d, i) => (
                    <div key={i} style={{ padding: '8px 0', borderBottom: '1px solid var(--border-soft)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ color: 'var(--text-muted)', fontSize: '12px' }}>{d.date || d.period}</span>
                      <span style={{ color: 'var(--text)', fontSize: '12px' }}>{((d.input_tokens || 0) + (d.output_tokens || 0)).toLocaleString()} tokens</span>
                    </div>
                  ))}
                </>
              )}
