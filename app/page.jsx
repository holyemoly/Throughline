...
// ── Sidebar ─────────────────────────────────────────────────────
// ── Usage Panel ─────────────────────────────────────────────────
function UsagePanel({ onClose, selectedModel, setSelectedModel, thinkingEnabled, setThinkingEnabled, googleConnected }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/lastfm').then(r => r.json()).catch(() => null),
      fetch('/api/calendar').then(r => r.json()).catch(() => null),
    ]).then(([usageData, musicData, calData]) => {
      setUsage(usageData);
      setMusic(musicData);
      setLoading(false);
    });
  ...
  const totalCost = usage?.data
    ? usage.data.reduce((sum, d) => {
        const inputCost = (d.input_tokens || 0) * 0.000003;
        const outputCost = (d.output_tokens || 0) * 0.000015;
        return sum + inputCost + outputCost;
      }, 0)
    : null;

  return (
    <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '300px', background: 'var(--bg-2)', borderLeft: '1px solid var(--border)', zIndex: 100, display: 'flex', flexDirection: 'column', animation: 'slideRight 0.2s ease' }}>
      <div style={{ padding: '20px 16px 12px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '18px', fontWeight: 300, color: 'var(--purple)' }}>usage & now playing</span>
        <button onClick={onClose} style={{ color: 'var(--text-dim)', fontSize: '18px' }}>✕</button>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        {loading ? (
          <p style={{ color: 'var(--text-dim)', fontSize: '13px', fontStyle: 'italic' }}>loading...</p>
        ) : (
          <>
            {/* Model switcher */}
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <button onClick={() => setThinkingEnabled(p => !p)} style={{ width: '100%', padding: '10px 14px', borderRadius: '10px', background: thinkingEnabled ? 'rgba(107,141,214,0.15)' : 'var(--bg-3)', border: `1px solid ${thinkingEnabled ? 'rgba(107,141,214,0.4)' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ width: '36px', height: '20px', borderRadius: '10px', background: thinkingEnabled ? '#6b8dd6' : 'var(--border)', position: 'relative', transition: 'background 0.2s' }}>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              {googleConnected ? (
                ...
                <a href="/api/google" style={{ display: 'block', padding: '10px 14px', borderRadius: '10px', background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--purple)', fontSize: '13px', textDecoration: 'none', textAlign: 'center' }}>
            ...
            {music && !music.error && (
              <div style={{ marginBottom: '20px' }}>
                ...
                {music.nowPlaying && (
                  <div style={{ padding: '10px 14px', borderRadius: '12px', background: 'rgba(155,114,207,0.1)', border: '1px solid rgba(155,114,207,0.2)', marginBottom: '8px' }}>
                    <div style={{ fontSize: '10px', color: 'var(--purple)', marginBottom: '4px', letterSpacing: '0.05em' }}>▶ now playing</div>
                ...
                {music.recentTracks?.length > 0 && (
                  <div>
                    <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '6px' }}>recently played</div>
                    {music.recentTracks.map((t, i) => (
                      <div key={i} style={{ padding: '6px 0', borderBottom: '1px solid var(--border-soft)', display: 'flex', flexDirection: 'column' }}>
                        <span style={{ color: 'var(--text)', fontSize: '12px' }}>{t.name}</span>
                        <span style={{ color: 'var(--text-dim)', fontSize: '11px' }}>{t.artist}</span>
                      </div>
                    ))}
                  </div>
                )}
                {music.topArtists?.length > 0 && (
                  <div style={{ marginTop: '12px' }}>
                    <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '6px' }}>top artists this week</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                      {music.topArtists.map((a, i) => (
                        <span key={i} style={{ padding: '3px 10px', borderRadius: '10px', background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--text-muted)', fontSize: '11px' }}>{a}</span>
                      ))}
                    </div>
                  </div>
                )}
            ...
            <div>
              ...
              <a href="https://console.anthropic.com/settings/usage" target="_blank" rel="noopener noreferrer"
                style={{ display: 'block', padding: '12px 14px', borderRadius: '12px', background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--purple)', fontSize: '13px', textDecoration: 'none', textAlign: 'center' }}>
                view usage in anthropic console →
              </a>
              <p style={{ color: 'var(--text-dim)', fontSize: '11px', marginTop: '8px', fontStyle: 'italic', lineHeight: 1.5 }}>
                sonnet 4.6: ~$3/million input tokens, ~$15/million output tokens. with prompt caching, repeated system prompt tokens cost 90% less.
              </p>
            </div>
          </>
        )}
...
export default function Home() {
  ...
  const [tooPersonal, setTooPersonal] = useState(false);
 ...
 const loadConversation = async (conv) => {
  ...
  const sendMessage = async (overrideInput) => {
    ...
    try {
      const res = await fetch('/api/chat', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: trimmed, attachments: userMessage.attachments, mode, conversationId: conv.id, folderId: currentFolder?.id || null, model: selectedModel, thinkingEnabled }),
      });
      const data = await res.json();
      if (data.message) {
  ...
  const handleModeSwitch = (newMode) => {
    ...
    setMessages([]); setWasTruncated(false); setSidebarOpen(false); setDocsOpen(false); setUsageOpen(false); setAttachments([]);
  };
  ...
  return (
    <>
      ...
      {usageOpen && (
        <>
          <div onClick={() => setUsageOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 99, backdropFilter: 'blur(2px)' }} />
          <UsagePanel onClose={() => setUsageOpen(false)} selectedModel={selectedModel} setSelectedModel={setSelectedModel} thinkingEnabled={thinkingEnabled} setThinkingEnabled={setThinkingEnabled} googleConnected={false} />
        </>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', maxWidth: '680px', margin: '0 auto' }}>
        ...
        <div style={{ padding: '12px 16px 10px', borderBottom: '1px solid var(--border-soft)', background: 'rgba(27,24,40,0.97)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 10, display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <button onClick={() => setUsageOpen(true)} style={{ color: 'var(--text-dim)', fontSize: '16px', flexShrink: 0, padding: '4px', opacity: 0.6 }}>◎</button>
        ...
        {/* Quick actions */}
        <div style={{ padding: '6px 16px 0', background: 'rgba(27,24,40,0.97)', display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button onClick={() => sendMessage("I'd rather not talk about that.")} style={{ fontSize: '11px', color: 'var(--text-dim)', padding: '3px 10px', borderRadius: '10px', border: '1px solid var(--border)', background: 'transparent', letterSpacing: '0.02em' }}>
            too personal
          </button>
          {thinkingEnabled && (
            <span style={{ fontSize: '10px', color: '#6b8dd6', fontStyle: 'italic', letterSpacing: '0.04em' }}>✦ thinking mode on</span>
          )}
          {selectedModel !== 'claude-sonnet-4-6' && (
            <span style={{ fontSize: '10px', color: 'var(--amber)', fontStyle: 'italic', letterSpacing: '0.04em' }}>
              {selectedModel.includes('opus') ? 'opus' : 'haiku'}
            </span>
          )}
        </div>
