  ...
  return (
    <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '320px', background: 'var(--bg-2)', borderLeft: '1px solid var(--border)', zIndex: 100, display: 'flex', flexDirection: 'column', animation: 'slideRight 0.2s ease' }}>
      ...
      <div style={{ padding: '12px', borderBottom: '1px solid var(--border-soft)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: editingInstructions ? '8px' : 0 }}>
          <span style={{ fontSize: '10px', color: 'var(--text-dim)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>system instructions</span>
          <button onClick={() => setEditingInstructions(e => !e)} style={{ color: folderColor, fontSize: '11px' }}>{editingInstructions ? 'cancel' : (instructions ? 'edit' : '+ add')}</button>
        </div>
        {editingInstructions ? (
          <>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '11px', lineHeight: 1.5, marginTop: '6px', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{instructions}</p>
        ) : null}
      </div>
...
// ── Sidebar ─────────────────────────────────────────────────────
function FolderItem({ folder, currentFolderId, onToggle, onNewConv, onDelete, onUpdate, folderConvs }) {
  const [editing, setEditing] = useState(false);
