...
// ── Settings Panel ──────────────────────────────────────────────
function SettingsPanel({ onClose, selectedModel, setSelectedModel, thinkingEnabled, setThinkingEnabled, contextSize, setContextSize }) {
  ...
  const sendMessage = async (overrideInput) => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ message: trimmed, attachments: userMessage.attachments, mode, conversationId: conv.id, folderId: currentFolder?.id || null, model: selectedModel, thinkingEnabled, contextSize }),
      });
      const data = await res.json();
      if (data.message) {
