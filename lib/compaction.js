...
export async function maybeCompactConversation(conversationId) {
  // If this conversation is already being compacted, skip
  if (activeCompactions.get(conversationId)) {
    return { compacted: false, reason: 'compaction already in progress' };
  }

  // Don't re-compact if we just compacted this conversation recently
  const { data: lastCompaction } = await supabaseAdmin
  ...
  if (lastCompaction) {
    const minutesSince = (Date.now() - new Date(lastCompaction.created_at).getTime()) / (1000 * 60);
    if (minutesSince < 10) {
      return { compacted: false, reason: `cooldown (${Math.round(minutesSince)}min since last)` };
    }
  }

  activeCompactions.set(conversationId, true);
  try {
